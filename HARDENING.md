...
- Why it matters: the proxy path is inert today (no env), but the moment Atlas is wired a shallow/incomplete upstream object can render `~$NaN` or crash the demo's `doc.actions.map` with no error boundary, instead of the intended `502`. Migrated here from `PATTERNS.md`.
- Effort: M
