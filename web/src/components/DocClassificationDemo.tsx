      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-foreground/50 mt-1">
            Live API call to <code className="text-foreground/70">/api/demo/classify-doc</code>. Pick a sample to run the pipeline.
          </p>
        </div>
        {stage !== 'idle' && (
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-4">
        <div className="rounded-lg border border-white/10 bg-white/[0.02] p-5">
          ...
          <div className="mt-4 text-[11px] font-mono text-foreground/35 leading-relaxed">
            File upload UX is identical in production. The backend swap is the only change: parse → LLM classifier → router.
          </div>
        </div>
        ...
        <div className="rounded-lg border border-white/10 bg-white/[0.02] p-5">
          ...
          {!error && stage === 'idle' && (
            <div className="rounded-md border border-white/5 bg-black/30 p-4 text-sm text-foreground/45 leading-relaxed">
              Pick a sample document to see the parse → classify → route pipeline run end-to-end.
            </div>
          )}
