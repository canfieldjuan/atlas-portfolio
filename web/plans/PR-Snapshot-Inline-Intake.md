...
3. `rg -n "sourceOffer: 'hero_intake'|top_blind_spots|unresolved_topic_count|resolved_topic_count" web/src web/scripts web/plans/PR-Snapshot-Inline-Intake.md` - confirm the rejected source offer and deferred schema migration are absent from runtime source; remaining hits are the smoke-test negative assertion and this plan's deferred/verification text.
4. `npm --prefix web run lint` - verify no eslint errors.
5. `npm --prefix web run build` - verify successful Next compilation.
6. `bash scripts/local_pr_review.sh` - run full PR review suite.
